...
public class SistemaClimaEfectos : MonoBehaviour
{
    ...
    // Velocidad nominal del tren (capturada una vez — evita decaimiento
    // exponencial al multiplicar la velocidad ya reducida en cada tick)
    float _velocidadNominalTren = -1f;
    ...
    void AjustarVelocidadTren(SistemaClima.EstadoClima clima)
    {
        ...
        // Capturar la velocidad nominal UNA vez (antes: tren.velocidadCrucero * 0.65
        // en cada tick → la velocidad decaía exponencialmente hacia 0 durante la nieve)
        if (_velocidadNominalTren < 0f)
            _velocidadNominalTren = tren.velocidadCrucero;
        ...
        tren.velocidadCrucero = clima == SistemaClima.EstadoClima.NieveLigera
            ? _velocidadNominalTren * 0.65f
            : _velocidadNominalTren;
    }
}

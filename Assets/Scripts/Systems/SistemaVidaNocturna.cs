...
public class SistemaVidaNocturna : SingletonMono<SistemaVidaNocturna>
{
    ...
    void ActualizarFarolas()
    {
        ...
        for (int i = 0; i < _farolas.Count; i++)
        {
            ...
            var hd = _hdFarolas[i];
            if (hd != null) hd.SetIntensity(intensidad, UnityEngine.Rendering.LightUnit.Lux);
    ...
    // FIX: era `void OnDestroy()`, que ocultaba el privado de SingletonMono<T> → Instance nunca
    // se anulaba → MissingReferenceException tras recarga (mismo patrón que SistemaManifestacion).

...
#pragma warning restore CS0414
    ...
    [Tooltip("GPS real de Herriko Plaza (para CesiumGeoreference).\n" +
             "Única fuente de verdad: GeoDataAlsasua (derivado del origen UTM).")]
    [SerializeField] private double latitudCentro  = GeoDataAlsasua.LATITUD_CENTRO;   // 42.898703
    [SerializeField] private double longitudCentro = GeoDataAlsasua.LONGITUD_CENTRO;  // -2.167699
    [SerializeField] private double altitudCentro  = 523.0;
    ...
    {
        // REUTILIZAR el georef existente (creado por ConfiguradorCesiumAlsasua
        // o la escena). Crear un segundo georef aquí duplicaba tilesets que
        // escapaban del excluder de CesiumFondoLejano y desviaba el GPS.
        _georef = FindFirstObjectByType<CesiumGeoreference>();
        if (_georef != null)
        {
            AlsasuaLogger.Info("CesiumCapas",
                "CesiumGeoreference existente reutilizado (lo alinea CesiumFondoLejano).");
            return;
        }

        _georef = gameObject.AddComponent<CesiumGeoreference>();
        _georef.latitude  = latitudCentro;    // 42.898703° N (Herriko Plaza, = origen UTM)
        _georef.longitude = longitudCentro;   // -2.167699° W
        _georef.height    = altitudCentro;    // 523 m s.n.m. (calibra CesiumFondoLejano)

        // Anclar en las coordenadas Unity de la plaza, no en (0,0,0)
        transform.position = new Vector3(GeoDataAlsasua.OX,
            (float)altitudCentro - GeoDataAlsasua.Z_MIN, GeoDataAlsasua.OZ);

        AlsasuaLogger.Info("CesiumCapas",
            $"CesiumGeoreference creado: {latitudCentro}°N {longitudCentro}°W @{altitudCentro}m");
    }
    ...
    {
        // Dedupe por ionAssetID — los nombres difieren entre creadores
        // (p. ej. "Google_Photorealistic_3DTiles" del configurador editor).
        foreach (var ts in FindObjectsByType<Cesium3DTileset>(
                     FindObjectsInactive.Include, FindObjectsSortMode.None))
        {
            if (ts.ionAssetID != assetId) continue;
            AlsasuaLogger.Info("CesiumCapas",
                $"Tileset ID:{assetId} ya existe ('{ts.name}') — no se duplica.");
            return;
        }

        var go  = new GameObject(nombre);
        // Colgar SIEMPRE del georef (local 0,0,0 = Herriko Plaza) para que el
        // excluder de CesiumFondoLejano mida distancias correctamente.
        go.transform.SetParent(_georef != null ? _georef.transform : transform, false);
    ...
    [Tooltip("Mostrar el panel de diagnóstico OnGUI (cuesta FPS en editor).")]
    [SerializeField] private bool mostrarDebugGUI = false;

    private void OnGUI()
    {
        if (!mostrarDebugGUI) return;

...
public class SistemaAguaRio : MonoBehaviour
{
    ...
    float _timerBuscaClima; // OPT: reintento espaciado, no Find por frame
    ...
    void Update()
    {
        ...
        // OPT: FindFirstObjectByType cada frame era coste fijo si SistemaClima
        // no existe en la escena — reintentar solo cada 2 s.
        if (_clima == null)
        {
            _timerBuscaClima -= Time.deltaTime;
            if (_timerBuscaClima > 0f) return;
            _timerBuscaClima = 2f;
            _clima = FindFirstObjectByType<SistemaClima>();
            if (_clima == null) return;
        }

        bool tormenta = _clima.climaActual == SistemaClima.EstadoClima.Tormenta;
        bool lluvia   = tormenta
        ...
        // OPT: si _t ya convergió, no reescribir las propiedades del agua.
        if (Mathf.Approximately(_t, tPrev)) return;

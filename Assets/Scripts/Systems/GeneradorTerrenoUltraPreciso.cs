...
public class GeneradorTerrenoUltraPreciso : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        ...
        // MOSAICO V2: las alturas vienen bakeadas del pipeline Python (lattice
        // 1/64, costuras bit-exactas) — aplicar aquí el resample del LIDAR las
        // DESTRUIRÍA. Este componente queda en modo SOLO-VALIDACIÓN (RMSE).
    ...
    IEnumerator ValidarTerrenoCoroutine()
    {
        ...
        if (File.Exists(pathGnd))
        {
                ...
                foreach (var line in File.ReadLines(pathGnd))
                {
                    ...
                    if (float.TryParse(tok[0], System.Globalization.NumberStyles.Float, ci, out float x)
                     ...
                     && float.TryParse(tok[2], System.Globalization.NumberStyles.Float, ci, out float z))
                        ...
                        lista.Add(new Vector3(x, y - Z_MIN, z));
                }
                // El xyz puede venir en Unity ABSOLUTO (v2: x≈1400-2400, plaza 1918)
                // o relativo a la plaza (legacy: x≈±512). Autodetección por mediana.
                if (lista.Count > 0)
                {
                    float mx = lista[lista.Count / 2].x;
                    if (Mathf.Abs(mx) < 600f) // relativo → pasar a absoluto
                        for (int i = 0; i < lista.Count; i++)
                            lista[i] += new Vector3(UNITY_OX, 0f, UNITY_OZ);
                }
                return lista;
            });
            ...
            if (taskXyz.IsCompletedSuccessfully && taskXyz.Result.Count > 0)
            {
                ...
                // Calcular errores para todos los puntos (tile-aware con mosaico)
                var errores = new List<(Vector3 pos, float delta)>(pts.Count);
                int lote = 0;
                for (int i = 0; i < pts.Count; i++)
                {
                    float yT = TerrenoGlobal.AlturaMundo(pts[i].x, pts[i].z);
        ...
        // Cotas REALES (m s.n.m.). El muestreo del terrain devuelve altura en el
        // datum local (alt_real - Z_MIN), así que se reconvierte antes de comparar.
        var refsAlt = new (string nombre, Vector3 pos, float cotaRealEsperada)[]
        {
            ("Herriko Plaza",     new(GeoDataAlsasua.OX, 0, GeoDataAlsasua.OZ), GeoDataAlsasua.COTA_PLAZA),
            ("Estacion de tren",  new(1650f, 0, 8200f), 533f),
            ("N. Carretera N-1",  new(1900f, 0, 8000f), 530f),
        };
        foreach (var (nombre, pos, esp) in refsAlt)
        {
            float cotaReal = terrain.SampleHeight(new Vector3(pos.x, 0, pos.z))
                           + terrain.transform.position.y + GeoDataAlsasua.Z_MIN;
            float err = cotaReal - esp;
            AlsasuaLogger.Info("Validacion",
                $"  {nombre}: {cotaReal:F1}m (esperado~{esp:F0}m, err={err:+F1;-F1}m)");
        }

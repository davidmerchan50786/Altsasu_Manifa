...
public class GeneradorInterioresSimples : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // OPT: barrido rotatorio — 64 edificios por frame en vez de todos.
        // Barrido completo ~0,27 s a 60 fps: imperceptible para los radios usados.
        int n = _edificios.Count;
        if (n == 0) return;
        int procesar = Mathf.Min(64, n);
    ...
    int _cursorEdificios; // OPT: cursor del barrido rotatorio
    ...
    static void ConfigurarLuz(Light luz, string arquetipo)
    {
        ...
        switch (arquetipo)
        {
            case "Bar":
                ...
                luz.range  = 8f;
                if (hd != null) hd.SetIntensity(2500f, UnityEngine.Rendering.LightUnit.Lux);
                break;
            case "Comercio":
                luz.color  = ColorTemperatura(4000f);
                luz.range  = 6f;
                if (hd != null) hd.SetIntensity(1800f, UnityEngine.Rendering.LightUnit.Lux);
                break;
            case "Industrial":
                luz.color  = ColorTemperatura(5000f);
                luz.range  = 4f;
                if (hd != null) hd.SetIntensity(800f, UnityEngine.Rendering.LightUnit.Lux);
                break;
            default: // Residencial
                luz.color  = ColorTemperatura(3200f);
                luz.range  = 5f;
                if (hd != null) hd.SetIntensity(1200f, UnityEngine.Rendering.LightUnit.Lux);
                break;
        }
    }

...
public sealed class GeneradorCallesAltsasu : MonoBehaviour
{
    ...
    [Header("Coexistencia con datos OSM reales")]
    [Tooltip("Si roads_unity.json existe (400 ejes OSM reales vía GeneradorMundoOSM), " +
             "este generador legacy de 8 calles aproximadas NO genera nada. " +
             "Evita calles duplicadas y desalineadas con las reales.")]
    public bool cederAnteOSM = true;
    ...
    void Start()
    {
        if (cederAnteOSM && System.IO.File.Exists(
                System.IO.Path.Combine(Application.dataPath, "AlsasuaData", "roads_unity.json")))
        {
            AlsasuaLogger.Info("CallesLegacy",
                "roads_unity.json presente — calles reales OSM activas, generador legacy en reposo.");
            return;
        }

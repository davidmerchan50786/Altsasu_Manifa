...
/// <summary>
/// OptimizadorMallaOBJ — Gestiona el renderizado eficiente del terreno importado
...
public class OptimizadorMallaOBJ : MonoBehaviour
{
    // ─── Configuración LOD ────────────────────────────────────────────────────
    ...
    void Start()
    {
        ...
        if (usarChunks && (chunksTerreno == null || chunksTerreno.Length == 0))
        {
            ...
            if (chunksTerreno.Length > 0)
                Debug.Log($"[OptimizadorMallaOBJ] Auto-detectados {chunksTerreno.Length} chunks.");
        }
    ...
    void AplicarTexturaCCOlor()
    {
        ...
        Debug.Log("[OptimizadorMallaOBJ] Textura CloudCompare aplicada: " + texturaCCOlor.name);
    }
    ...
    void MarcarComoStatic()
    {
        ...
        Debug.Log("[OptimizadorMallaOBJ] Objetos marcados como Static correctamente.");
    }
    ...
    void InfoMalla()
    {
        ...
        {
            Debug.LogWarning("[OptimizadorMallaOBJ] No hay MeshFilter o malla asignada.");
            return;
        }
        var mesh = mf.sharedMesh;
        Debug.Log($"[OptimizadorMallaOBJ] === INFO MALLA CloudCompare ===\n" +
                  $"  Nombre       : {mesh.name}\n" +

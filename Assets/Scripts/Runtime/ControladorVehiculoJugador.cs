...
public class ControladorVehiculoJugador : VehiculoBase, IInteractable
{
    ...
    // velocidadMax se HEREDA de VehiculoBase (protected + serializado, aparece
    // en el Inspector). Ocultarla aquí con `new` compilaba, pero la serialización
    // de Unity NO soporta el mismo nombre de campo en clase e hija:
    //   "The same field name is serialized multiple times… velocidadMax"
    // El default de coche jugador (25 m/s ≈ 90 km/h) se fija en Reset().
    [Tooltip("Tracción: true = 4WD (Montero off-road), false = RWD (coches normales).")]
    [SerializeField] private bool  cuatroRuedas       = false;

    void Reset() { velocidadMax = 25f; } // default al añadir el componente en editor

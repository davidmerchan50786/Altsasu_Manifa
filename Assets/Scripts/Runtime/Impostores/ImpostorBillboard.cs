...
public class ImpostorBillboard : MonoBehaviour
{
    ...
    public long id;
    [Tooltip("Si lo dirige GestorImpostores, no se orienta solo en LateUpdate.")]
    ...
    void Inicializar()
    {
        ...
        _ok = true;
        Orientar(Camera.main); // primera pose
    }

    void LateUpdate() { if (!gestionado) Orientar(Camera.main); }

    /// <summary>Reasigna este billboard a otro edificio (para reuso por el pool).</summary>
    public void Reasignar(ImpostorAtlasSO a, long nuevoId)
    {
        atlas = a; id = nuevoId; _ok = false;
        gameObject.name = $"Impostor_{nuevoId}";
        Inicializar();
    }

    /// <summary>Orienta el quad a 'cam' y fija la vista yaw. Público para el gestor.</summary>
    public void Orientar(Camera cam)
    {
        if (!_ok || cam == null) return;

...
public class SistemaParanoiaGuardiaCivil : MonoBehaviour
{
    ...
    [Tooltip("Panel único de tuning. Si se asigna, manda sobre el umbral/factor del config.")]
    ...
    // ── Para el HUD ───────────────────────────────────────────────────────────
    public float Paranoia => _paranoia;
    public int Convertidos()
    { int n = 0; for (int i = 0; i < _todos.Count; i++) if (_todos[i] && _todos[i].Convertido) n++; return n; }
    ...
    void Update()
    {
        ...
        // El apoyo popular FRENA la conversión: calle alta = menos guardias.
        float ap  = SistemaApoyoPopular.Instance != null ? SistemaApoyoPopular.Instance.apoyo : 0f;
        int objNpc, objCoche;
        if (sintonia != null)
        {
            // El panel da el total con su umbral + FactorApoyo + maxConvertidos;
            // lo repartimos npc:coche según los topes del config.
            int tot = sintonia.ConvertidosObjetivo(_paranoia, ap);
            int sum = Mathf.Max(1, config.maxNpc + config.maxCoches);
            objNpc   = Mathf.RoundToInt(tot * (config.maxNpc / (float)sum));
            objCoche = Mathf.Clamp(tot - objNpc, 0, config.maxCoches);
            objNpc   = Mathf.Clamp(objNpc, 0, config.maxNpc);
        }
        else
        {
            float fap = config.FactorApoyo(ap);
            objNpc    = Mathf.RoundToInt(config.Objetivo(_paranoia, config.maxNpc)   * fap);
            objCoche  = Mathf.RoundToInt(config.Objetivo(_paranoia, config.maxCoches) * fap);
        }
    ...
    {
        if (actual < objetivo)   // convertir uno (SOLO off-screen: Convertir devuelve false si visible)
        {
            var c = ElegirNoConvertidoOffscreen(coche);
            if (c != null && c.Convertir(config)) return true;
        }
        else if (actual > objetivo)  // revertir uno (SOLO off-screen)
        {
